    ...
    <div class="card">
      ...
      <div class="input-wrapper">
        ...
        <input
          ...
          @keydown.enter="check"
        />
        <button class="Subscribe-btn" @click="check">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="30"
            height="10"
...
const phone = ref("");
const check = () => {
  if (!phone.value || !/^\d+$/.test(phone.value) || phone.value.length !== 11) {
    alert("请输入正确的电话号码");
  } else {
    // 跳转到home页面，存入ls
    query.equalTo("phone", phone.value);
